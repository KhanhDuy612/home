...
export default function BannerSlider() {
  const { data: home } = useApiQuery<any>('/items/banner');
  const slides = home?.data?.bg_img;

  console.log('Banner Slider Data:', slides);

  if (!slides || slides.length === 0) {
    return <div className="text-center text-white">No banner data available</div>;
  }
  return (
    <Swiper
